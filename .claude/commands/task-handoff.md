...
2. 同时调 MCP `record_session` 保留可恢复会话：`issue=$ARGUMENTS[0]`，`session_id=${CLAUDE_SESSION_ID}`（为空则用 `$TASKBOARD_SESSION_ID`），`agent=claude-code`；如刚切过分支，先用 Bash 取 `git branch --show-current` 并透传 `branch`，并传 `work_dir=$(pwd)`。
