...
export default function SettingsPanel({ settings, onSaved, onClose }: Props) {
  ...
  const confirmEditCol = (accountId: number) => {
    ...
    const updated =
      index >= 0 ? st.columns.map((c, i) => (i === index ? newCol : c)) : [...st.columns, newCol];
  ...
  const toggleRule = (accountId: number, v: string) => {
    ...
    updateAccountState(accountId, {
      ruleChips: st.ruleChips.includes(vv)
        ? st.ruleChips.filter((x) => x !== vv)
        : [...st.ruleChips, vv],
    });
  };
    ...
    <div className="modal-mask" onClick={onClose}>
      ...
      >
        ...
        <div style={{ display: 'flex', gap: 6, marginBottom: 14, flexWrap: 'wrap' }}>
          {(['base', 'columns', 'diagnose'] as SettingsTab[])
            ...
            <div className="row" style={{ marginTop: 4 }}>
              ...
              <button
                ...
                onClick={() => {
                  ...
                  if (diagAccountId) void loadProjects(diagAccountId);
                }}
                ...
                {projects.map((p) => (
                  <div key={p.id} className="project-row">
                    ...
                    <span className="muted small">
                      {p.numberOfItems} items · {p.ownerType}
                    </span>
                  </div>
                ))}
              </div>
        ...
        <div
          style={{ display: tab === 'columns' && CUSTOM_COLUMN_MAPPING_ENABLED ? 'block' : 'none' }}
        >
          <div className="field">
            <label>{t('settings.boardModeTitle')}</label>
            <div className="muted small" style={{ marginBottom: 8 }}>
              {t('settings.boardModeDesc')}
            </div>
          </div>
            ...
            {settings.accounts.map((acct) => (
              <AccountCard
                ...
                onUpdateEditingCol={(patch) => {
                  ...
                  if (st?.editingCol)
                    updateAccountState(acct.id, { editingCol: { ...st.editingCol, ...patch } });
                }}
...
function AccountCard({
  ...
  return (
    ...
    >
      <div
        className="account-config-header"
      ...
      >
        ...
        <span style={{ marginLeft: 'auto' }} className="muted small">
          {t('settings.boardModeLabel')}:
        </span>
        <select
          className="select"
          value={boardMode}
          onChange={(e) => void handleBoardModeChange(e.target.value as BoardMode)}
          style={{ width: 'auto' }}
        >
      ...
      {boardMode === 'custom' && (
        <div
          className="account-config-columns"
        ...
        >
            ...
            >
              <div className="row" style={{ gap: 6, marginBottom: 6 }}>
                <div style={{ flex: 1 }}>
                  ...
                  <input
                    className="input"
                    placeholder={t('settings.customColumns.colNamePlaceholder')}
                    value={state.editingCol.colName}
                    onChange={(e) => onUpdateEditingCol({ colName: e.target.value })}
                  />
                </div>
              </div>
              <div style={{ marginBottom: 6 }}>
                    ...
                    {state.availableStatuses.map((s) => {
                      ...
                      return (
                        <button
                          key={`st_${s}`}
                          type="button"
                          onClick={() => !disabled && onToggleRule(s)}
                          disabled={disabled}
                ...
                <div className="row" style={{ gap: 6, marginTop: 6 }}>
                  <input
                    ...
                    onChange={(e) => onUpdateRuleInput(e.target.value)}
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') {
                        e.preventDefault();
                        onAddCustomRule();
                  ...
                  />
                  <button className="btn ghost small" onClick={onAddCustomRule}>
              ...
              <div className="row" style={{ gap: 6 }}>
                <button
                  ...
                  onClick={onConfirmEditCol}
                  disabled={!state.editingCol.colName.trim()}
                >
                  {t('btn.done')}
                </button>
                <button className="btn" onClick={onCancelEditCol}>
                  {t('btn.cancel')}
                </button>
              </div>
            </div>
          ...
          {state.columns.map((col, idx) => (
            ...
            >
              <span className="chip" style={{ minWidth: 24, textAlign: 'center' }}>
                {idx}
              </span>
              ...
              >
                {(() => {
                  try {
                    ...
                    return col.matchRules;
                  }
                })()}
              </span>
              <button className="btn ghost small" onClick={() => onStartEditCol(col, idx)}>
                {t('settings.customColumns.edit')}
              </button>
              <button className="btn ghost small" onClick={() => onDeleteCol(idx)}>
          ...
          <div className="row" style={{ gap: 6, marginTop: 8 }}>
            <button className="btn" onClick={onStartAddCol}>
              + {t('settings.customColumns.add')}
            </button>
            <button className="btn primary" onClick={onSaveColumns} disabled={state.saving}>
              {state.saving ? t('settings.loading') : t('btn.save')}
            </button>
            {state.msg && (
              <span
                className={`muted small ${state.msg === t('settings.customColumns.saved') ? 'ok' : 'error'}`}
              >
                {state.msg}
              </span>
            )}
