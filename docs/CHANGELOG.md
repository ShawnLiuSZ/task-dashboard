...
- **未发布（Unreleased）— 左右分栏布局 + Agent 接入面板（#259）**
  ...
  - **#259 记事本四列布局修正**：修复后四列（紧急 / 高 / 中 / 低）在真实窗口下**从来不是并排的**——`tauri.conf.json` 默认 1180×760 下紧接/高占第一行、中/低被挤到第二行，1440 下 3+1，900 下直接叠成 4 行；容器只写 `overflow-y: auto`，按 CSS 规则横向会被**计算成 auto**，因此还带上了内部横向滚动条。同时「收起」按钮收的是**整个面板**（四列一并消失）。详见 [docs/issue-259-sidebar-nav.md](./issue-259-sidebar-nav.md)（「追加修正：记事本四列并排」一节）。
  - **做法**：按看板 `.column` 的既有模式改造——`.notes-card-cols` 去 `flex-wrap` + `overflow: hidden`，`.note-col` 改 `flex: 1 1 0` + `min-width: 0`（等宽可收缩、永不换行），纵向滚动下沉到每列 `.note-col-body`；**收起粒度从整面板改为只收起创建列**（收起态渲染 30px `.notes-add-rail` 作为四列容器的兄弟节点），空数据时始终渲染四列（不再用整块空状态替换），创建列 textarea 撑满列高；另修窄列（~90px）下 `.note-foot` 撑宽卡片导致的**列内横向滚动条**（`.note-card { min-width: 0 }` + `.note-foot { flex-wrap: wrap }`，换行收缩而非裁掉溢出）；顺带删除随之失效的 `.notes-panel.collapsed` / `.notes-rail*` / `.notes-add-col-open` / `.notes-empty*` 死代码与 4 个无用 i18n key（中英各 338）。纯前端，SQLite / Rust 零改动。
  - **验证**：隔离复现页（真实 `styles.css` + 复刻 DOM）+ 无头 Chrome 截图 + 页面内注入读数：900/1180/1440 三档四列 `top/bottom` 全部齐平、容器 `overflowX=hidden`、横向溢出元素 0 个、textarea 与列高等高；新增回归测试 `app/src/components/notes-layout.test.ts` 8 例并**反向验证**（改回 `flex-wrap: wrap` / `flex: 0 0 260px` / 四列内引用 `addColCollapsed` 三处缺陷写法，对应 4 条断言全部失败）。
