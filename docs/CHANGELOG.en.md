...
- **Unreleased — Notes page: four side-by-side columns + create-column-only collapse (#259)**

  - **#259 Notes four-column layout fix**: after the split into "create column + four columns", the four columns were **never actually side by side** in a real window — at the `tauri.conf.json` default of 1180×760 Urgent/High sat on the first row while Medium/Low were pushed to a second one; at 1440 it became 3+1; at 900 they stacked into 4 rows. The container only declared `overflow-y: auto`, and per the CSS rules `overflow-x: visible` **computes to auto**, so it also carried an internal horizontal scrollbar. On top of that the "collapse" button collapsed the **entire panel**, taking the four columns with it. See [docs/issue-259-sidebar-nav.md](./issue-259-sidebar-nav.md) (section "追加修正：记事本四列并排").
  - **How**: rebuilt along the existing board `.column` pattern — `.notes-card-cols` lost `flex-wrap` and gained `overflow: hidden`; `.note-col` became `flex: 1 1 0` + `min-width: 0` (equal-share, shrinkable, never wrapping) with vertical scrolling pushed down into each `.note-col-body`; the **collapse granularity changed from the whole panel to the create column only** (the collapsed state renders a 30px `.notes-add-rail` as a sibling of the four-column area), the four columns always render even with zero notes, and the create-column textarea fills the column height. Also fixed the **in-column horizontal scrollbar** that appeared at narrow widths (~90px per column) when `.note-foot` widened the card (`.note-card { min-width: 0 }` + `.note-foot { flex-wrap: wrap }` — wrap and shrink rather than clip the overflow). Dead CSS that went with it was removed: `.notes-panel.collapsed` / `.notes-rail*` / `.notes-add-col-open` / `.notes-empty*`, plus 4 now-unused i18n keys (338 keys per locale). Frontend only — SQLite / Rust untouched.
  - **Verification**: isolated repro page (real `styles.css` + replicated DOM) plus headless-Chrome screenshots and in-page readouts: at 900/1180/1440 the four columns share identical top/bottom, the container reports `overflowX=hidden`, zero horizontally overflowing elements anywhere in the tree, and the textarea matches the column height; new regression test `app/src/components/notes-layout.test.ts` (9 cases) **reverse-verified** (reintroducing `flex-wrap: wrap`, `flex: 0 0 260px`, an `addColCollapsed` reference inside the four-column block, or dropping `min-width: 0` / `flex-wrap: wrap` fails the matching assertions).
