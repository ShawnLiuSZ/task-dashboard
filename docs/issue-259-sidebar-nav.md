...
> **命名说明**：该功能在界面上名为**「备忘录」**（2026-09-16 由「记事本」改名，见 `CHANGELOG.md`）。
> 代码标识、类名与测试仍沿用 `notes` / `NotesPanel` / `.note-*`，本文档沿用同一套标识，
> 仅在描述界面文案处使用「备忘录」。
...
### 第五轮修正（同日）：移除整行页头，导入/导出挪进创建列；列头间距统一

1. **整行页头删除**（记事本标题 + 计数 + 导入/导出 + 收起）：标题与侧边栏重复、白占一行
   纵向空间。`.notes-head*` / `.notes-tools` CSS 一并删除
   （`.notes-head-icon` 仍被 AgentPanel 复用，保留）。
2. **导入/导出挪到创建列顶部工具行**（24px）：左侧「收起创建列 ‹」、右侧「导出 ⬇ / 导入 ⬆」
   （含隐藏 file input）。列收起时随列隐藏，导轨可恢复。
3. **列头→内容间距统一**：旧值列头 `margin-bottom: 2px` + 列体 `padding-top: 0`
   ⇒ 有记录的列上边距只有 2px，与空列（提示自带 padding）观感不一致。
   改为列头 `margin-bottom: 0`、列体 `padding-top: 8px`，空列提示去独立内边距
   ⇒ 有记录/没有记录的列上边距完全一致；`.notes-body` 内边距对齐看板（`12px 16px 16px`）。

验证：`notes-layout.test.ts` 19 例；反向验证 3 类缺陷（页头类回归、工具行挪出创建列、
间距回退）⇒ 3 条断言全部失败。
⚠️ 注入脚本教训续：锚点不能选「标识符首次出现处」——`fileInputRef` 首次出现是在它的
...
### 第八轮（同日）：记事本列与看板列视觉统一（宽度 + 列头样式）

用户要求：记事本的四列（紧急/高/中/低）改用**账号面板（看板）列的样式与宽度**——
两处面板的列应当看起来是同一套设计。

做法（逐条对齐 `.column` / `.column-head` / `.column-body`，不再各写一套）：

| 项 | 看板 `.column` | 记事本 `.note-col`（改后，同参） |
|---|---|---|
| 列宽 | 固定 `320px`（`flex: 0 0 320px`） | 固定 `var(--notes-col-w)` = **320px**（与创建列共用变量） |
| 列头几何 | `margin: 8px 8px 2px`、`padding: 5px 10px`、`border-radius: 7px` | 同 |
| 列头底色 | 按状态取浅色（`#dbe9fc` / `#e6e7ea` / amber 变量） | 按优先级取**同一套**浅色（紧急 `#fde7ec`、高 `var(--amber-bg)`、中 `#dbe9fc`、低 `#e6e7ea`） |
| 列头内容 | 圆点 + 标题（`font-weight: 500`）+ 右侧灰色计数 | 同（原为「彩色计数徽章 + 标题内嵌 `::before` 圆点」） |
| 列体 | `padding: 0 8px 10px`、`gap: 8px` | 同 |

- 列类名改为 `note-col--<优先级>`（配合看板 `.empty` 作用域收紧后的命名约定）；
- **取消空列的灰化处理**：看板空列同样显示彩色列头与计数 0，为求统一而移除
  `note-col--empty` 相关规则（空列只在列体显示「暂无」提示）；
- 列宽从「≥280px 可伸缩」改回**固定 320px**：窄窗口下由 `.notes-card-cols` 出横向滚动条
  （与看板整窗横向滚动同一交互取向）。

验证：`notes-layout.test.ts` 24 例，新增「列宽与看板 `.column` 一致（含变量值 = 320px）」
「列头/列体几何逐条与看板同参」「列头内容同构」「四档列头浅色底」等断言；
反向验证一轮注入 7 处缺陷（宽度变量回 280、列回到可伸缩、列头几何回旧值、
列体内边距回旧值、删一档列头底色、空列类回归、删列头圆点）⇒ **5 条断言失败**，
还原后字节一致、24 例全绿。

### 第九轮（同日）：列头改为看板状态列写法（列顶色条 + 无胶囊底）

用户贴出看板列头截图（Backlog 列）：**列顶一条状态色横条 + 列头无底色**（圆点/标题/计数
直接落在列底色上），要求记事本照此改。第八轮做的「浅色胶囊列头」其实抄的是看板
**4 状态视图**的写法（`.column-todo .column-head { background }`），而账号面板列用的是
**状态列写法**：`.column-status-N { border-top: 3px solid var(--status-N) }`。

改动：

- `.note-col` 增加 `border-top: 3px solid var(--col-accent, var(--border))`
  —— 同看板状态列的 3px 状态色横条，颜色仍由组件按看板色板注入 `--col-accent`；
- `.note-col-head` **去掉背景**（删掉 4 条 `.note-col--<优先级> .note-col-head { background }`
  与 4 条标题字色规则）——状态色只落在列顶横条与圆点上，标题用默认文字色；
- 列头几何与列头内容（圆点 + 标题 + 右侧灰色计数）保持与看板同参。

验证：`notes-layout.test.ts` 24 例，断言改为「列顶 3px 状态色横条 + 列头不得有底色
+ 标题不得按优先级上色」（并与看板 `.column-status-0` 的写法对照断言）；
反向验证（去掉横条 / 加回胶囊底 / 标题上色 ⇒ 断言失败，还原后字节一致）。
