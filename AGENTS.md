...
### 2.1 产品约束：默认不写回 GitHub（指 TaskBoard 同步方向）
...
- 诊断 / 排查默认用 `gh` 只读命令。
- 看板状态变更走本地 MCP 工具（`update_task_status` 等），不通过改 GitHub label / state 实现；**MCP 工具保持只写本地**（写回仅限 UI 确认框触发）。
- 维护者明确要求起草或修改 issue 正文、PR 描述、评论等协作文字时，允许执行；未经明确要求，不得创建 / 关闭 / 改状态 / 改 label。
